...
function getURL(variable,cycle,stat,domain,fhr,time_period){
...
//	for(i=0; i<levels.length; i++){
		...
		var urls = getURL(obj.variable,obj.cycle,obj.stat,obj.domain,obj.fhr,obj.time_period);
...
function pressRight(){
	var curDomain = searchByName(imageObj.domain,domains);
	if(curDomain < domains.length-1){curDomain += 1; changeDomain(domains[curDomain].name);}
}

function pressLeft(){
	var curDomain = searchByName(imageObj.domain,domains);
	if(curDomain > 0){curDomain = curDomain - 1; changeDomain(domains[curDomain].name);}
}
