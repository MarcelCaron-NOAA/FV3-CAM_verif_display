...
function getURL(level,variable,domain,stat,cycle,fhr,time_period){
...
//	for(i=0; i<levels.length; i++){
		...
		var urls = getURL(obj.level,obj.variable,obj.domain,obj.stat,obj.cycle,obj.fhr,obj.time_period);
...
//Change the vertical level/threshold from dropdown menu
...
function pressDown(){
        if (imageObj.indep == "series_initdate_sfc"){
...
function pressRight(){
	var curFhr = searchByName(imageObj.fhr,fhrs);
	if(curFhr < fhrs.length-1){curFhr += 1; changeFhr(fhrs[curFhr].name);}
}

function pressLeft(){
	var curFhr = searchByName(imageObj.fhr,fhrs);
	if(curFhr > 0){curFhr = curFhr - 1; changeFhr(fhrs[curFhr].name);}
}
